...
export default function UserSupportChat() {
    ...
    useEffect(() => {
        if (socket && registered && chatStatus === "none"){
            startSupportChat()
        }

    }, [socket, registered, chatStatus])
    ...
    const backToAicon = () => {
        const abort = confirm(`このチャットを終了してAIコンに戻ります。`)
        if (abort){
    ...
    return (
        ...
        <div>
            <div className="flex flex-row justify-between gap-x-4 mx-5 my-5">
        ...
        {chatStatus !== 'none' && (
            <>
            <div className="flex-none h-[64vh] w-11/12 mx-auto max-w-96 overflow-auto bg-yellow-100">
